<x-layouts.admin title="Edit Kartu Keluarga" header="Edit Kartu Keluarga">
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 max-w-4xl">
        <form action="{{ route('admin.kartu-keluargas.update', $kartuKeluarga) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="rt_id"
                    ...
                    @foreach ($rts as $rt)
                        <option value="{{ $rt->id }}" @selected(old('rt_id', $kartuKeluarga->rt_id) == $rt->id)>
            ...
            <div>
                ...
                <textarea name="alamat"
                          rows="4"
                          class="w-full rounded-xl border-slate-300 focus:border-indigo-500 focus:ring-indigo-500">{{ old('alamat', $kartuKeluarga->alamat) }}</textarea>
            </div>
            ...
            <div class="flex items-center gap-3">
                <input type="checkbox"
                       ...
                       @checked(old('is_active', $kartuKeluarga->is_active))>
